...
<script type="text/javascript">
	$(document).ready(function(){
     	...
     	var filter = [];console.log("filter="+filter);
        $('.parameter:checked').each(function(){
            filter.push($(this).val());
        });
      $.ajax({
         url:"<?php echo url(''); ?>/studentdetails?filters="+filter,
  ...
  function fetch_data(page, sort_type="", sort_by="", search="") {
      $.ajax({
         url:"/studentdetails-ajax?page="+page+"&sorttype="+sort_type+"&sortby="+sort_by+"&search="+search,
